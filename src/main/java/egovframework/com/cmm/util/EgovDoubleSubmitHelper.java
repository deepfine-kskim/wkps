...
public class EgovDoubleSubmitHelper {
	...
	public static boolean checkAndSaveToken(String tokenKey) {
		...
		if (session.getAttribute(EgovDoubleSubmitHelper.SESSION_TOKEN_KEY) == null) {
			try {
				throw new EgovComException("Double Submit Preventer TagLig isn't set. Check JSP.");
		...
		if (parameter == null) {
			try {
				throw new EgovComException("Double Submit Preventer parameter isn't set. Check JSP.");
			} catch (EgovComException e) {
	        	LOGGER.error("[" + e.getClass() +"] :" + e.getMessage());
			}
		}
