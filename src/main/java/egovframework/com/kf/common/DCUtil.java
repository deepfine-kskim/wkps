...
@Component("dcUtil")
public class DCUtil {
    private static final Logger DCUTIL_LOGGER = LoggerFactory.getLogger(DCUtil.class);
     ...
     * @return 쿼리 StringBuffer
     */
    public StringBuffer makeAONQuery(String nmFd, String kwd, String schMethod,
                                     StringBuffer query, String logicOp) {
...
//		System.out.print("@@@@@prevKwd : ");
        for (int i = 1; i < prevKwd.length; i++) {
//			System.out.print(prevKwd[i]);
        }
//		System.out.println();
        if (prevKwd != null && prevKwdLength >= 1) {
            for (int i = 1; i < prevKwdLength; i++) {
                if (!escapeQuery(prevKwd[i]).equalsIgnoreCase(kwd)) {
                    if (query.length() > 0) {
                        query.append(" AND ");
//						System.out.println("진입했따!!!!!!!!!!! : "+ prevKwd[i]);
                    }
                    query.append(nmFd);
                    query.append("='");
...
/*	public StringBuffer makeMasterQuery(String[][] queryValue, ParameterVO param){
     ...
     */
    public String getRestURLDetail(SearchVO searchVO) throws UnsupportedEncodingException {
        StringBuffer url = new StringBuffer();
        url.append(searchVO.getUrl());
        String param1 = "?select=" + searchVO.getFields();
        url.append(param1);
        String param2 = "&from=" + searchVO.getFrom();
        url.append(param2);
        String param3 = "&where=" + searchVO.getQuery();
        url.append(param3);
        url.append("&limit=1");
        String param4 = "&charset=" + searchVO.getCharset();
        url.append(param4);
        String param5 = "&hilite-fields=" + searchVO.getHilightTxt();
        url.append(param5);
        url.append("&default-hilite=off");
        return url.toString();
    }
    ...
    public String getRestURL(ParameterVO paramVO, SearchVO searchVO) throws UnsupportedEncodingException {
        ...
        String param4 = "&limit=" + paramVO.getPageSize();
        url.append(param4);
        String param5 = "&charset=" + searchVO.getCharset();
        url.append(param5);
        String param6 = "&hilite-fields=" + urlEncode(searchVO.getHilightTxt(), null);
        url.append(param6);
        url.append("&syn-domain-no=0");
        String param7 = "&custom=" + searchVO.getLogInfo();
        url.append(param7);
        return url.toString();
    }
    ...
    public String getParamPostData(ParameterVO paramVO, SearchVO searchVO) throws UnsupportedEncodingException {
        ...
        String param5 = "&charset=" + searchVO.getCharset();
        url.append(param5);
        String param6 = "&hilite-fields=" + searchVO.getHilightTxt();
        url.append(param6);
        String param7 = "&custom=" + searchVO.getLogInfo();
        url.append(param7);
        return url.toString();
    ...
    public StringBuffer getUrlData(String urlinfo, String charset) {
        ...
        try {
            ...
            DCUTIL_LOGGER.error("konan search engine search error...", ie);
            ...
            try {
                ...
                DCUTIL_LOGGER.error("konan search engine search br.close()...", ie);
            }
            if (conn != null) {
                conn.disconnect();
            }
     ...
     * @throws UnsupportedEncodingException
     */
    public String getKsmURL(Map<String, String> map) throws UnsupportedEncodingException {
        StringBuffer url = new StringBuffer();
        url.append(map.get("klaUrl"));
        String param1 = "?data.jsp?site=" + map.get("site"); 
        url.append(param1);
        String param2 = "&keyword=" + map.get("keyword"); 
        url.append(param2);
        url.append("&target=docid_rank&format=logflow");
        String param3 = "&fromDate=" + map.get("fromDate"); 
        url.append(param3);
        String param4 = "&toDate=" + map.get("toDate"); 
        url.append(param4);
        url.append("&count=30&output=csv");
        return url.toString();
    }
}
