...
<c:if test="${edmsPostTotal > 0}">
<div class="section_sch_result">
	<div class="result_head">
	<!-- 09-16 리뷰용 수정 -->
		<strong>전자결재 생산함</strong> <span class="text-primary">(총 <fmt:formatNumber value="${edmsPostTotal}" groupingUsed="true"/>건)</span>
	</div>
	...
	<!-- //09-16 리뷰용 수정 -->
	<c:if test="${edmsPostTotal > params.pageSize}">
	  <c:choose>
	  	<c:when test="${params.category ne 'edmsPost'}">
	<div class="more_btn_area">
		<a href="javascript:goCategory('edmsPost');" class="btn btn-warning outline">검색 결과 더보기</a>
	</div>
...
<script> 
	if(${params.category eq 'edmsPost'}){
		document.write(pageNav('gotopage','${params.pageNum}','10','${edmsPostTotal}','1'));
	}
</script>
</c:if>          
