...
public class SrhEdmsRecvDAO {
	...
	public int SrhRecv(ParameterVO paramVO, Model model) throws Exception {
	int rc=0,total=0;
	long hc;
  ...
  //검색권한조건
    ...
    try {
    	...
    	if(rc>=0) {
    		...
    		model.addAttribute("edmsRecvTotal", crz.GetResult_TotalCount(hc));
    		...
    		//model.addAttribute("edmsRecvTotal", dpbt1_cnt);
    		model.addAttribute("edmsRecvList", result);
