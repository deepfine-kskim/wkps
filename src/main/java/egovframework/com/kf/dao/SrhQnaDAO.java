 ...
 */
@Repository
public class SrhQnaDAO {
	private static final Logger SRH_QNA_LOGGER = LoggerFactory.getLogger(SrhQnaDAO.class);
...
//		logger.debug("db query>> "+query.toString());
		...
		searchVO.setFields(konanPropertiesService.getString("qnaField"));
		searchVO.setFrom(konanPropertiesService.getString("qnaFrom"));
		...
		SRH_QNA_LOGGER.debug("qna restUrl>>"+restUrl);
		...
		if(!success) {
			SRH_QNA_LOGGER.error("dbDao error>>>");
			return null;
		}
		return restVO;		
	}
