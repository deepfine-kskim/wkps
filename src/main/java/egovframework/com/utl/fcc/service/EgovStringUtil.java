...
public class EgovStringUtil {
	...
	public static String stripStart(String str, String stripChars) {
		...
		if (stripChars == null) {
			...
			while (stripChars.indexOf(str.charAt(start)) != -1) {
				start++;
			}
		}
	...
	/**
	 * 스트링의 가운데 반을 *을 붙여서 리턴하는 유틸
	 * @param text 입력받은 문자
	 * @return 가운데에 *를 붙여서 리턴한다.
	 */
	public static String StringHalfMasking(String text) {
		String value = text;
		if(org.apache.commons.lang3.StringUtils.isNotBlank(value)) {
			Integer maskingLength = value.length() / 2;
			String masking = "*";
			for (int j = 1; j < maskingLength; j++) {
				masking += "*";
			}
			value = value.substring(0, maskingLength) + masking;
		}
		return value;
	}
}
