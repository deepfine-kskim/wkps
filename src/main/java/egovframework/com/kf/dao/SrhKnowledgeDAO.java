 ...
 */
@Repository
public class SrhKnowledgeDAO {
    private static final Logger SRH_KNOW_LOGGER = LoggerFactory.getLogger(SrhKnowledgeDAO.class);
...
//		logger.debug("db query>> "+query.toString());
        ...
        searchVO.setFields(konanPropertiesService.getString("knowledgeField"));
        searchVO.setFrom(konanPropertiesService.getString("knowledgeFrom"));
        ...
        SRH_KNOW_LOGGER.debug("knowledge restUrl>>" + restUrl);
        ...
        if (!success) {
            SRH_KNOW_LOGGER.error("dbDao error>>>");
            return null;
        }
        return restVO;
    }
