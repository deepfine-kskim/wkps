 ...
 */
@Repository
public class SrhSurveyDAO {
	private static final Logger SRH_SURVEY_LOGGER = LoggerFactory.getLogger(SrhSurveyDAO.class);
...
//		logger.debug("db query>> "+query.toString());
		...
		searchVO.setFields(konanPropertiesService.getString("surveyField"));
		searchVO.setFrom(konanPropertiesService.getString("surveyFrom"));
		searchVO.setHilightTxt(konanPropertiesService.getString("surveyHilight"));
		...
		SRH_SURVEY_LOGGER.debug("survey restUrl>>"+restUrl);
		...
		if(!success) {
			SRH_SURVEY_LOGGER.error("dbDao error>>>");
			return null;
		}
		return restVO;		
	}
