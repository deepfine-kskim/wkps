...
<%--                    <p class="mb_5"><span class="text-danger">※</span> <span class="underline text-black">일자 클릭 시 시간대별 현황</span>을 보실 수 있습니다.  당일 데이터는 명일 확인 가능합니다.</p>--%>
                    <table class="table table-bordered text-center table-hover">
                        <caption class="sr-only">Q&A 통계 리스트</caption>
                        <colgroup>
                            <col style="width:10%;">
                            <col>
                            <col>
                        ...
                        <thead>
                            <tr>
                                <th scope="col">일자</th>
                                <th scope="col">질문 등록수</th>
                                <th scope="col">전일 대비 질문 등록수</th>
                                <th scope="col">답변 등록수</th>
                                <th scope="col">전일 대비 방문 횟수 </th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <!-- 루프 -->
                            <c:choose>
                                <c:when test="${!empty statics.staticsQnaVoList && fn:length(statics.staticsQnaVoList) > 0}">
                                    <c:forEach var="item" items="${statics.staticsQnaVoList}">
                                        <tr>
                                            <td>${item.dt}</td>
                                            <td>${item.questionCount}</td>
                                            <td><span class="${item.preQuestionCount > 0 ? 'text-danger': item.preQuestionCount < 0 ? 'text-primary':''}">${item.preQuestionCount}</span></td>
                                            <td>${item.answerCount}</td>
                                            <td><span class="${item.preAnswerCount > 0 ? 'text-danger':item.preAnswerCount < 0 ?'text-primary':''}">${item.preAnswerCount}</span></td>
                                        </tr>
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                        ...
                        <tfoot>
                            <tr class="danger text-black">
                                ...
                                <td>
                                    <strong>${statics.totalQuestionCount}</strong>
                                </td>
                                <td>
                                    <strong>${statics.totalPreQuestionCount}</strong>
                                </td>
                                <td>
                                    <strong>${statics.totalAnswerCountCount}</strong>
                                </td>
                                <td>
                                    <strong>${statics.totalPreAnswerCountCount}</strong>
                                </td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        $(".dev-download-excel").on("click", function() {
                            ...
                            form.attr("action", "/adm/statQnaExcelDownload.do");
                        ...
                        $(".dev-search").on("click", function() {
                            var form = $("form[name=searchForm]");
                            form.attr("action", "/adm/statQna.do");
