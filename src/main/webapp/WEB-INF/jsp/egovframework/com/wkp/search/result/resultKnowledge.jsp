...
<c:if test="${knoTotal > 0}">
<div class="section_sch_result">
	<div class="result_head">
	<!-- 09-16 리뷰용 수정 -->
		<strong>지식백과</strong> <span class="text-primary">(총 <fmt:formatNumber value="${knoTotal}" groupingUsed="true"/>건)</span>
	</div>
	<ul class="sch_result_list">
		<c:forEach var="result" items="${knoList}" varStatus="status">
		<li>
			...
			<div class="data_box">
				...
				<div class="file_area">
					<div class="file">
						...
						<a href="javascript:;" class="btn btn-xs btn-default outline preview" data-url="http://105.0.1.229${result.file_stre_cours }${result.stre_file_nm }" data-fid="KNO_${result.knowlg_no }">미리보기</a>
					</div>
				</div>
				</c:if>
			</div>
	...
	<!-- //09-16 리뷰용 수정 -->
	<c:if test="${knoTotal > params.pageSize}">
	  <c:choose>
	  	<c:when test="${params.category ne 'kno'}">
	<div class="more_btn_area">
		<a href="javascript:goCategory('kno');" class="btn btn-warning outline">검색 결과 더보기</a>
	</div>
...
<script> 
	if(${params.category eq 'kno'}){
		document.write(pageNav('gotopage','${params.pageNum}','10','${knoTotal}','1'));
	}
</script>
</c:if>          
