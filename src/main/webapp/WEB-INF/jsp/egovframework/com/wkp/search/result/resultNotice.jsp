...
<div class="section_sch_result">
	...
	<!-- 09-16 리뷰용 수정 -->
		<strong>공지사항</strong> <span class="text-primary">(총 <fmt:formatNumber value="${noticeTotal}" groupingUsed="true"/>건)</span>
	</div>
			...
			<div class="data_box">
				...
				<div class="file_area">
					<div class="file">
						...
						<a href="javascript:;" class="btn btn-xs btn-default outline preview" data-url="http://105.0.1.229${result.file_stre_cours }${result.stre_file_nm }" data-fid="NOTICE_${result.notice_no }">미리보기</a>
					</div>
				</div>
				</c:if>
			</div>
	...
	<!-- //09-16 리뷰용 수정 -->
	<c:if test="${noticeTotal > params.pageSize}">
	  <c:choose>
	  	<c:when test="${params.category ne 'notice'}">
	<div class="more_btn_area">
		<a href="javascript:goCategory('notice');" class="btn btn-warning outline">검색 결과 더보기</a>
	</div>
...
<script> 
	if(${params.category eq 'notice'}){
		document.write(pageNav('gotopage','${params.pageNum}','10','${noticeTotal}','1'));
	}
</script>
</c:if>                                             