...
public class EgovNetworkState {
			...
			if ("WINDOWS".equals(Globals.OS_TYPE)) {
				...
				while (true) {
					...
					if (str.length() >= MAX_STR_LEN) {
						throw new EgovComException("input too long");
					}
					if (!str.trim().equals("")) {
						processes.add(str);
					}
	...
	public static String getMyIPaddress() {
		try {
			...
			if (!EgovWebUtil.isIPAddress(InetAddress.getLocalHost().getHostAddress())) {
				throw new EgovComException("IP is needed. (" + InetAddress.getLocalHost().getHostAddress() + ")");
			}
	 ...
	 * @see
	 */
	public static boolean getPingTest(String requestIP) {

		boolean status = false;
		
		try {			
			if (!EgovWebUtil.isIPAddress(requestIP)) {
				throw new EgovComException("IP is needed. (" + requestIP + ")");
			}
