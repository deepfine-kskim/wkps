 ...
 */
@Repository
public class SrhNoticeDAO {
    private static final Logger SRH_NOTICE_LOGGER = LoggerFactory.getLogger(SrhNoticeDAO.class);
...
//			System.out.println("as "+Arrays.asList(preKwds));
			/*
			if(preKwds.length > 0) {
				query.insert(0, "( ");
				query.append(" )  ");
...
//		logger.debug("db query>> "+query.toString());
        ...
        searchVO.setFields(konanPropertiesService.getString("noticeField"));
        searchVO.setFrom(konanPropertiesService.getString("noticeFrom"));
        ...
        SRH_NOTICE_LOGGER.debug("notice restUrl>>" + restUrl);
        ...
        if (!success) {
            SRH_NOTICE_LOGGER.error("dbDao error>>>");
            return null;
        }
        return restVO;
    }
