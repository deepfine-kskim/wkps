 ...
 */
@Repository
public class SrhCommunityDAO {
    private static final Logger SRH_COMM_LOGGER = LoggerFactory.getLogger(SrhCommunityDAO.class);
...
//		logger.debug("db query>> "+query.toString());
        ...
        searchVO.setFields(konanPropertiesService.getString("communityField"));
        searchVO.setFrom(konanPropertiesService.getString("communityFrom"));
        searchVO.setHilightTxt(konanPropertiesService.getString("communityHilight"));
        ...
        SRH_COMM_LOGGER.debug("community restUrl>>" + restUrl);


        RestResultVO restVO = new RestResultVO();
        //문서만 검색시 문서가 없으므로 null리턴
        ...
        if (!success) {
            SRH_COMM_LOGGER.error("dbDao error>>>");
            return null;
        }
