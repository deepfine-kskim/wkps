...
$(function() {
    ...
    $('#btn_save_comment').click(function(){
		...
		$.post("writeCommunity2Comment.do",
			   param, 
    ...
    $('#btn_del').click(function(){  	
		...
		$.post("deleteCommunity2Free.do",
		   ...
		   function(data, status) {
			var json = JSON.parse(data);
		      if(json.success){
		    	location.href = "/cmu/community2FreeList.do?cmmntyNo="+cmmntyNo;
 			  } else{
 				 alert(json.err_msg);
 			  } 
			} 
...
function del_comment(commentNo){
	...
	$.post("deleteCommunity2Comment.do",
		   param, 
	   function(data, status) {
		var json = JSON.parse(data);
	      if(json.success){
...
function mod_comment(commentNo){
	...
	$.post("updateCommunity2Comment.do",
		   param, 
	   function(data, status) {
		var json = JSON.parse(data);
	      if(json.success){
                    ...
                    <div id="contents" class="col-md-9">
                        ...
                        <div class="page-body">
                            ...
                            <ul class="list-group post_nav">
                            	<c:if test="${prev != null }">
                                <li class="list-group-item"><a href="community2FreeView.do?cmmntyNo=${community.cmmntyNo }&pstgNo=${prev.pstgNo}" class="text-overflow dblong"><strong><i class="fa fa-chevron-circle-left" aria-hidden="true"></i> 이전글</strong> : ${prev.title } </a></li>
                                </c:if>
                                <c:if test="${next != null }">
                                <li class="list-group-item"><a href="community2FreeView.do?cmmntyNo=${community.cmmntyNo }&pstgNo=${next.pstgNo}" class="text-overflow dblong"><strong><i class="fa fa-chevron-circle-right" aria-hidden="true"></i> 다음글</strong> : ${next.title }</a></li>
                                </c:if>
                            </ul>
                            <!-- //이전/다음 -->
