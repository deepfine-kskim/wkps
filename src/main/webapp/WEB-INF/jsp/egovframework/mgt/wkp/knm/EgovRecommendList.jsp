...
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
        <div class="cont_wrap">
            <div class="cont_header">
                <div class="row">
                    <div class="col-xs-6">
                        <h2 class="page_title">추천 지식 관리</h2>
                    </div>
            ...
            <div class="cont_body">
                    ...
                    <div class="tab-content">
                    	<div role="tabpanel" class="tab-pane active" id="bbsTab1">
		                    <table class="table text-center table-bordered table-hover brd_list">
		                        ...
		                        <tbody>
		                        	...
		                        	<c:if test="${recommend.knowlgMapType eq 'REPORT'}">
			                        <tr>
			                            <td>${recommend.rki }</td>
			                            <td><span class="text-warning">${recommend.knowlgMapType }</span></td>
			                            <td class="text-left">
			                                <p class="subject">
			                                    <a href="#">${recommend.title }</a>
			                                </p>
			                            </td>
			                            <td>${recommend.registerId }</td>
			                            <td><span class="text-blue">${recommend.recomendCnt }</span></td>
			                            <td><span class="text-black">${recommend.inqCnt }</span></td>
			                            <td>${recommend.registDtm }</td>
			                            <td><a href="/adm/deleteRecommend.do?title=${recommend.title}&rki=${recommend.rki }&knowlgMapType=${recommend.knowlgMapType }&recomendTypeCd=RECOMMEND" class="btn btn-default" onclick="return confirm('삭제하시겠습니까?');">삭제</a></td>
			                        </tr>
			                        </c:if>
			                        </c:forEach>
		                        </tbody>
                		...
                		<div role="tabpanel" class="tab-pane" id="bbsTab2">
		                    <table class="table text-center table-bordered table-hover brd_list">
		                        ...
		                        <tbody>
		                        	<c:forEach var="recommend" items="${recommendList }">
		                        	<c:if test="${recommend.knowlgMapType eq 'REFERENCE'}">
			                        <tr>
			                            <td>${recommend.rki }</td>
			                            <td><span class="text-warning">${recommend.knowlgMapType }</span></td>
			                            <td class="text-left">
                		...
                		<div role="tabpanel" class="tab-pane" id="bbsTab3">
		                    <table class="table text-center table-bordered table-hover brd_list">
		                        ...
		                        <tbody>
		                        	<c:forEach var="recommend" items="${recommendList }">
		                        	<c:if test="${recommend.knowlgMapType eq 'PERSONAL'}">
			                        <tr>
			                            <td>${recommend.rki }</td>
			                            <td><span class="text-warning">${recommend.knowlgMapType }</span></td>
			                            <td class="text-left">
			                            ...
			                            <td><span class="text-black">${recommend.inqCnt }</span></td>
			                            <td>${recommend.registDtm }</td>
			                            <c:url var="url" value="/adm/deleteRecommend.do" >
			                            	<c:param name="rki" value="${recommend.rki }" />
			                            	<c:param name="knowlgMapType" value="${recommend.knowlgMapType }" />
			                            	<c:param name="recomendTypeCd" value="RECOMMEND" />
			                            </c:url>
			                            <td><a href="${url }" class="btn btn-default" onclick="return confirm('삭제하시겠습니까?');">삭제</a></td>
			                        </tr>
			                        </c:if>
			                        </c:forEach>
		                        </tbody>
                ...
                <div id="newRegPopup" class="modal fade" role="dialog" aria-labelledby="newRegPopupLabel">
                    <div class="modal-dialog" role="document">
                        <form:form id="recommendFrm" class="modal-content form-horizontal" action="/adm/insertRecommend.do" modelAttribute="recommendVO">
                        	<input type="hidden" name="recomendTypeCd" value="RECOMMEND">
